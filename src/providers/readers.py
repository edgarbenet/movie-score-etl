...
def extract_csv(path: Path) -> list[dict]:
    logger.info(indent(color(
        f"{ICONS['extract']} Reading CSV {bold(path.name)}",
        CYAN,
    )))
    rows = []
    ...
    logger.info(indent(
        color(f"{ICONS['ok']} {len(rows)} rows parsed", GREEN)
    ))
    return rows
...
def extract_json(path: Path) -> list[dict]:
    logger.info(indent(color(
        f"{ICONS['extract']} Reading JSON {bold(path.name)}",
        CYAN,
    )))
    ...
    logger.info(indent(
        color(f"{ICONS['ok']} {len(rows)} items loaded", GREEN)
    ))
    return rows
...
def extract_from_path(path: Path) -> list[dict]:
    ...
    logger.info(indent(
        color(f"{ICONS['dispatch']} Dispatch based on suffix {suffix}", YELLOW)
    ))
